...
<div class="flex flex-nowrap items-center text-lg">
  {#if typeof entry === "string"}
    <pre
      data-theme="dark"
      data-prefix="$"
      class="grow text-wrap pl-8! -indent-8 break-words"><code>{entry}</code
      ></pre>
  {:else if entry.prefix === ""}
    <pre
      data-theme="dark"
      class="before:mr-0! ml-6! grow text-wrap break-words {entry.cl}"><code
        >{entry.text}</code
      ></pre>
  {:else}
    <pre
      ...
      class="grow text-wrap pl-8! -indent-8 break-words {entry.cl}"><code
        >{entry.text}</code
      ></pre>
  {/if}
