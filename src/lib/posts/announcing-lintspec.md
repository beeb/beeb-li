...
Speed was an important factor because I wanted people to be able to use `lintpsec` in git pre-commit hooks, where any
command running for more than a few milliseconds really affects the developer experience.
...
The action even generates some annotations on the code for PRs, which appear when reviewing it.
...
Thanks to the JSON output generated by the tool, it's easy to extract information about the found problems with tools
like [`jq`](https://jqlang.org/) and make your own if you don't use Github Actions.
