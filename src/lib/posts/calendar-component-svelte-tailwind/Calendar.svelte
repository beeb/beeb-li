<script lang="ts">
  ...
  const firstDayColumn = $derived(
    (new Date(year, month, 1).getDay() + 7 - firstDayOfWeek) % 7,
  );
  ...
  const dayNames = $derived(
    Array.from({ length: 7 }, (_, i) =>
      dateTimeFormat.format(new Date(2018, 0, i + firstDayOfWeek)),
    ),
  );
...
<div
  class="w-full max-w-lg mx-auto overflow-x-scroll card bg-base-200 shadow-sm p-4"
>
  <div class="min-w-80 flex flex-col gap-4">
    <div class="flex flex-nowrap items-center text-center sm:text-xl">
      <div>
        <button
          class="btn btn-ghost text-xl font-normal"
          onclick={() => month--}
    ...
    <div
      class="grid grid-cols-7 justify-items-center text-sm sm:text-base opacity-80"
    >
    ...
    <div class="grid grid-cols-7 justify-items-center">
      {#each range(1, lastDay) as day}
        <div
          class={`${colStartClass[firstDayColumn]} h-10 text-base sm:text-lg`}
        >
          {day}
        </div>
      {/each}
